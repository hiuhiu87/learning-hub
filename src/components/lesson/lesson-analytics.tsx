...
export default function LessonAnalytics({
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-slate-50 text-slate-900 transition-colors dark:bg-slate-950 dark:text-slate-100">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-sky-200/40 via-slate-50 to-white transition-colors dark:from-sky-600/20 dark:via-slate-950 dark:to-slate-950" />
      <div className="pointer-events-none absolute inset-0 opacity-30 [background:radial-gradient(rgba(148,163,184,0.12)_1px,transparent_1px)] [background-size:26px_26px]" />

      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-10 px-6 py-10 md:px-10">
        <header className="flex flex-col gap-6 rounded-3xl border border-slate-200/70 bg-white/80 p-6 shadow-2xl backdrop-blur transition-colors dark:border-white/10 dark:bg-white/5 md:flex-row md:items-center md:justify-between">
          <div className="flex items-start gap-4 md:items-center">
            ...
            <div>
              ...
              <h1 className="mt-1 text-3xl font-semibold text-slate-900 transition-colors dark:text-white">
                {lesson.title}
              </h1>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            iconClass="border-sky-400/40 bg-sky-100 text-sky-700 transition-colors dark:border-sky-400/30 dark:bg-sky-500/10 dark:text-sky-100"
          ...
          <StatCard
            ...
            iconClass="border-emerald-500/40 bg-emerald-100 text-emerald-700 transition-colors dark:border-emerald-400/30 dark:bg-emerald-500/10 dark:text-emerald-100"
          ...
          <StatCard
            ...
            iconClass="border-violet-500/40 bg-violet-100 text-violet-700 transition-colors dark:border-violet-400/30 dark:bg-violet-500/10 dark:text-violet-100"
            label="Average score"
            value={`${averageScore}%`}
          ...
          <StatCard
            ...
            iconClass="border-rose-500/40 bg-rose-100 text-rose-700 transition-colors dark:border-rose-400/30 dark:bg-rose-500/10 dark:text-rose-100"
            label="Needs review"
        ...
        <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <Card className="border-slate-200/70 bg-white/80 text-slate-900 shadow-2xl backdrop-blur transition-colors dark:border-white/10 dark:bg-slate-950/60 dark:text-slate-100">
            <CardHeader>
              <CardTitle className="text-xl font-semibold text-slate-900 transition-colors dark:text-white">
                Learner performance
              </CardTitle>
            ...
            <CardContent>
              {studentSummaries.length === 0 ? (
                <p className="text-sm text-slate-600 transition-colors dark:text-slate-300">
                  No learner activity yet. Share the lesson to get started.
                </p>
              ) : (
                  ...
                  <TableBody>
                    {studentSummaries.map(summary => (
                      <TableRow
                        ...
                        <TableCell className="text-slate-600 transition-colors dark:text-slate-400">
                          {getStudentEmail(summary) ?? "—"}
                        </TableCell>
                        <TableCell className="text-slate-600 transition-colors dark:text-slate-300">
                          {summary.attempts}
                        </TableCell>
                        <TableCell className="text-slate-600 transition-colors dark:text-slate-300">
                          {summary.correct}
                        ...
                        <TableCell className="text-right">
                          <Badge
                            variant="secondary"
                            className={`border-slate-200/70 bg-slate-100 text-slate-700 transition-colors dark:border-white/10 dark:bg-white/10 ${
                              summary.score >= 70
...
function InsightBlock({
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-slate-200/70 bg-white/80 p-4 text-sm text-slate-700 transition-colors dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
      <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200/70 bg-slate-100 text-slate-600 transition-colors dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
        {icon}
      </div>
      <div>
        <p className="font-semibold text-slate-900 transition-colors dark:text-white">
