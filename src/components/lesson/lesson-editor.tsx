...
import type { LessonQuestion } from "@/src/types/lesson";
...
export default function LessonEditor({
  ...
  const handleSave = async () => {
    ...
    if (
      ...
      toast.warning(
        "Please enter a positive number of minutes or leave blank."
      );
      return;
    }
    ...
    try {
      ...
      if (flashcards.length === 0 && questions.length === 0) {
        toast.warning("Please add at least one flashcard or question.");
        setIsSaving(false);
        return;
      }
      ...
      for (const flashcard of flashcards) {
        if (flashcard.id.startsWith("new-")) {
          if (flashcard.front.trim() === "" && flashcard.back.trim() === "") {
            toast.warning(
              "Flashcards cannot be empty. Please fill in the content or delete the flashcard."
            );
            setIsSaving(false);
            return;
          }
          ...
          <div className="flex items-start gap-4 md:items-center">
            <Link href="/dashboard">
              <Button
                variant="ghost"
                size="icon"
                className="text-slate-700 dark:text-white outline-none hover:bg-transparent dark:hover:bg-transparent transition-all hover:scale-140">
            ...
            <div>
              ...
              <h1 className="mt-1 text-3xl font-semibold text-slate-900 transition dark:text-white">
                {lessonId ? "Edit your lesson" : "Create a new lesson"}
              </h1>
