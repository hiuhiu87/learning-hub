...
export default function StudentDashboard({ userId }: { userId: string }) {
  ...
  const fetchLessons = useCallback(
    async (options?: { skipInitialSpinner?: boolean }) => {
      ...
      try {
        ...
        const lessonIds = enrollmentData?.map((entry) => entry.lesson_id) ?? []
        ...
        if (lessonIds.length > 0) {
          ...
          const sortedLessons =
            lessons?.slice().sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()) ?? []
          setEnrolledLessons(sortedLessons)
        } else {
          setEnrolledLessons([])
        }
        ...
        const sortedAvailable =
          allLessons?.slice().sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()) ?? []
        setAvailableLessons(sortedAvailable)
      } catch (error) {
        console.error("Error loading lessons", error)
        setDashboardError("We couldn’t load your lessons right now. Please try again.")
      } finally {
        setIsLoading(false)
  ...
  const handleEnroll = async (lessonId: string) => {
    ...
    try {
      ...
      setFeedback({
        type: "success",
        message: "You’ve joined the lesson! Jump in anytime from your active list.",
      })
    } catch (error) {
  ...
  const totalAvailable = useMemo(() => filteredAvailable.length, [filteredAvailable])
    ...
    <div className="relative min-h-screen overflow-hidden bg-slate-50 text-slate-900 transition-colors dark:bg-slate-950 dark:text-slate-100">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-10 px-6 py-10 md:px-10">
        <header className="flex flex-col gap-6 rounded-3xl border border-slate-200/60 bg-white/70 p-6 shadow-2xl backdrop-blur transition dark:border-white/10 dark:bg-white/5 md:flex-row md:items-center md:justify-between">
          <div className="flex items-start gap-4 md:items-center">
            ...
            <div>
              ...
              <h1 className="mt-1 text-3xl font-semibold text-slate-900 transition dark:text-white">Welcome back, learner</h1>
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-300">
                Pick up where you left off, explore new lessons, and keep your streak alive.
              </p>
            </div>
          </div>
        ...
        <section className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <Card className="border-slate-200/60 bg-white/80 text-slate-900 shadow-2xl backdrop-blur transition dark:border-white/10 dark:bg-white/10 dark:text-white">
            <CardHeader className="pb-2">
              <CardTitle className="text-2xl font-semibold text-slate-900 dark:text-white">Your current journey</CardTitle>
              <CardDescription className="text-slate-500 dark:text-slate-200">
                Stay consistent with daily practice and your next milestone will be closer than ever.
              </CardDescription>
            </CardHeader>
              ...
              {isLoading ? (
                ...
                <div className="space-y-5 rounded-3xl border border-slate-200/60 bg-slate-50/80 p-6 transition dark:border-white/10 dark:bg-slate-950/40">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <p className="text-xs font-medium uppercase tracking-[0.3em] text-slate-400 dark:text-slate-400">Next up</p>
                  ...
                  <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300">
                    {nextUpLesson.description || "Glance through the lesson outline and start when you’re ready."}
                  </p>
                  <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
                    <span className="rounded-full border border-slate-200/60 bg-white/70 px-3 py-1 transition dark:border-white/10 dark:bg-white/5">
                      Joined {formatDistanceToNow(new Date(nextUpLesson.created_at), { addSuffix: true })}
                    </span>
                    {nextUpLesson.time_limit_minutes != null && (
                      <span className="rounded-full border border-slate-200/60 bg-white/70 px-3 py-1 transition dark:border-white/10 dark:bg-white/5">
                        {nextUpLesson.time_limit_minutes} minute
                        {nextUpLesson.time_limit_minutes === 1 ? "" : "s"} focus session
                      </span>
                    )}
                    <Link
                ...
                <div className="rounded-3xl border border-dashed border-slate-300 bg-white/70 p-8 text-center text-slate-600 transition dark:border-white/15 dark:bg-white/5 dark:text-slate-300">
                  ...
                  <h2 className="mt-5 text-2xl font-semibold text-slate-900 dark:text-white">You haven&apos;t enrolled yet</h2>
                  <p className="mt-2 text-sm text-slate-500 dark:text-slate-300">
                    Discover lessons curated for you and start your first journey today.
                  </p>
                  <a
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
            <Card className="border-slate-200/60 bg-white/80 text-slate-900 backdrop-blur transition dark:border-white/10 dark:bg-slate-950/50 dark:text-white">
              <CardContent className="flex items-center gap-4 p-6">
                ...
                <div>
                  ...
                  <p className="text-3xl font-semibold text-slate-900 dark:text-white">{enrolledLessons.length}</p>
            ...
            <Card className="border-slate-200/60 bg-white/80 text-slate-900 backdrop-blur transition dark:border-white/10 dark:bg-slate-950/50 dark:text-white">
              <CardContent className="flex items-center gap-4 p-6">
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Available now</p>
                  <p className="text-3xl font-semibold text-slate-900 dark:text-white">{totalAvailable}</p>
                  <p className="text-sm text-slate-500 dark:text-slate-400">
        ...
        {enrolledLessons.length > 0 && (
          <section className="space-y-5">
            <div className="flex flex-col gap-2">
              <h2 className="text-2xl font-semibold text-slate-900 dark:text-white">My active lessons</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-1">
                        ...
                        <CardDescription className="line-clamp-3 text-slate-500 dark:text-slate-300">
                          {lesson.description || "This lesson is ready when you are."}
                        </CardDescription>
                      </div>
                  ...
                  <CardContent className="space-y-4">
                    {lesson.time_limit_minutes != null && (
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        ...
                        {lesson.time_limit_minutes === 1 ? "" : "s"} planned duration
                      </p>
                    )}
        ...
        <section id="discover-lessons" className="space-y-5">
          <div className="flex flex-col gap-2">
            <h2 className="text-2xl font-semibold text-slate-900 dark:text-white">Discover lessons</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              ...
              <Card className="border-dashed border-slate-300 bg-white/70 text-slate-600 transition dark:border-white/20 dark:bg-white/5 dark:text-white">
                <CardContent className="flex flex-col items-center gap-4 p-10 text-center">
                  ...
                  <p className="text-lg font-semibold">
                    {availableLessons.length === 0 ? "No lessons available yet." : "No lessons match your search."}
                  </p>
                  <p className="max-w-xl text-sm text-slate-500 dark:text-slate-300">
                    Check back soon—new lessons are added regularly to keep your learning path exciting.
                  </p>
                  <Button
                    variant="secondary"
                ...
                {filteredAvailable.map((lesson) => (
                  ...
                  >
                    ...
                    <CardHeader>
                      ...
                      <CardDescription className="line-clamp-3 text-slate-500 dark:text-slate-300">
                        {lesson.description || "A fresh lesson is ready for you to explore."}
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      {lesson.time_limit_minutes != null && (
                        <p className="text-xs text-slate-500 dark:text-slate-400">
                          {lesson.time_limit_minutes} minute
                          {lesson.time_limit_minutes === 1 ? "" : "s"} estimated time
                        </p>
                      )}
                      <Button
